...
using System.Linq;
using YourProject.Core.Domain;
using YourProject.Tests.Core.Helpers;
...
namespace Gramps.Tests.Core
{
    ...
    // ReSharper restore InconsistentNaming
    {
        ...
        protected void LoadExampless(int entriesToAdd)
        {
            for (int i = 0; i < entriesToAdd; i++)
            {
                var validEntity = CreateValidEntities.Example(i + 1);
                validEntity.Example = Repository.OfType<Example>().Queryable.First();
                Repository.OfType<Example>().EnsurePersistent(validEntity);
            }
        }
